...
formEl.addEventListener("submit", (e) => {
  ...
  L.marker(coordinates)
    .addTo(map)
    .bindPopup(`<b>Stroke ${strokes.length}</b>`)
    .openPopup();

  let tPoint = turf.point([coordinates[1], coordinates[0]]);
  console.log(">>> point", tPoint);
  console.log(
    ">>> distance (w/conversation)",
    turf.distance(tPoint, tCenter, { units: "miles" })
  );
...
function printMessage() {
  ...
  for (let i = 0; i < strokes.length; i++) {
    ...
    let distance = turf.distance(turf.point([stroke[1], stroke[0]]), tCenter, {
      units: "miles",
    });
    message += `Stroke ${i + 1}: ${(
